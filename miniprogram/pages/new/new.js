...
Page({
  ...
  deleteAllAddress(){
    ...
    wx.cloud.callFunction({
      name:"coffee_get_allAddressID",
      success:res=>{
        console.log("res ==>",res.result.data)
        var address = res.result.data
        address.forEach(v=>{
          ids.push(v._id)
        })
        console.log("ids ==>",ids)
        wx.cloud.callFunction({
          name:"coffee_delete_allAddress",
          data:{
            ids
          },
          success:res=>{
  ...
  getDefaultAddress: function (address) {
    ...
    wx.cloud.callFunction({
      name: 'coffee_get_address_default',
      success: res => {
        ...
        if (res.result.data.length > 0) {
          ...
          wx.cloud.callFunction({
            name: 'coffee_update_address_default',
            success: res => {
              console.log('res ==> ', res);
